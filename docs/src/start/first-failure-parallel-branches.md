...
First examine the current split in this resistive model:


![At a 15 V drop, a 165 A aggregate current passes the summed rating but its 150 A first-member current violates the 100 A limit.](../assets/start-here-same-ybus.png)
...
![Voltage-drop discs on one scale: source radius 10 V, aggregate radius 200/11 V, and witness at 15 V.](../assets/parallel-feasible-set-card.png)

The figure is generated by `experiments/render_parallel_feasible_set_card.py`.
The retained and aggregate regions are discs in the complex ``\Delta U``
plane, plotted in volts on identical horizontal and vertical scales. The
point ``(15,0)`` lies in the target’s extra region. No weighted or coupled
constraint is represented in this scalar plot.
