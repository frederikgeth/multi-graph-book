...
function five_bus_typed_kron_witness()
    ...
    reduced_edges = [edge for edge in edges if edge.id != "x"]
    ...
    edge_x = only(edge for edge in edges if edge.id == "x")
    line_x_current = edge_x.admittance * (nonpendant_full_voltage[4] - nonpendant_full_voltage[5])
    line_x_limit = max(abs(line_x_current) - 0.01, 0.0)
    checks = Dict(
        ...
        "eliminated_bus_is_pendant" => [edge.id for edge in edges if edge.bus_from == internal_vertex || edge.bus_to == internal_vertex] == ["x"],
        ...
        "recovered_line_x_current_is_exact" => abs(line_x_current - edge_x.admittance * (nonpendant_full_voltage[4] - nonpendant_full_voltage[5])) ≤ 1.0e-12,
        "tight_line_x_limit_is_not_satisfied" => abs(line_x_current) > line_x_limit,
    )
    Dict(
        ...
        "line_x_current" => Dict("real" => real(line_x_current), "imag" => imag(line_x_current)),
        "line_x_declared_limit" => line_x_limit,
        "line_x_limit_satisfied" => abs(line_x_current) ≤ line_x_limit,
