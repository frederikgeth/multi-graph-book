...
export typed_kron_fixture,
       ...
       restricted_transformer_library_fixture,
       assess_restricted_transformer_library,
       complex_pair,
...
"Build a restricted transformer-library witness with conductor-diagonal couplings."
function restricted_transformer_library_fixture(c = 2)
    source = typed_kron_fixture().Y_library
    target = zeros(ComplexF64, size(source))
    n = size(source, 1) ÷ c
    for p in 1:n
        for q in 1:n
            target[(p - 1) * c + 1:p * c, (q - 1) * c + 1:q * c] =
                Diagonal(diag(block(source, p, q, c)))
        end
    end
    target
end

"Check closure in a restricted transformer library with conductor-diagonal blocks."
function assess_restricted_transformer_library(YK, c; atol = 1e-12)
    n = size(YK, 1) ÷ c
    diagonal_blocks_diagonal = all(
        norm(block(YK, p, p, c) - Diagonal(diag(block(YK, p, p, c)))) ≤ atol
        for p in 1:n
    )
    off_diagonal_blocks_diagonal = all(
        norm(block(YK, p, q, c) - Diagonal(diag(block(YK, p, q, c)))) ≤ atol
        for p in 1:n for q in p+1:n
    )
    reciprocal = norm(YK - transpose(YK)) ≤ atol
    admissible = reciprocal && diagonal_blocks_diagonal && off_diagonal_blocks_diagonal
    reason = admissible ?
        "reciprocal relation has conductor-diagonal winding blocks" :
        "restricted transformer library rejects at least one dense conductor block"
    (; reciprocal, diagonal_blocks_diagonal, off_diagonal_blocks_diagonal, admissible, reason)
end
